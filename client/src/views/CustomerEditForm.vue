    ...
    <form class="add-form" @submit="submitForm">
        ...
        <div class="add-form__body">
            <form-group v-model="name" label-value="Imie i nazwisko" type-value="text" placeholder-value="Podaj nazwę"
                name-value="name" :default-value="name"></form-group>
            <form-group v-model="phoneNumber" label-value="Numer telefonu" type-value="number"
                placeholder-value="Podaj numer telefonu" name-value="phoneNumber" :default-value="phoneNumber">
            </form-group>
            <form-group-dropdown v-model="search" :search-value="search" label-value="Firma" type-value="text"
                placeholder-value="Wpisz nazwę" name-value="company" :default-value="companyName"
                :listDropdown="$store.getters.getFiltredCompanies(search)" @selected="elementClick"
                @reset="isCorrectSelected"></form-group-dropdown>
    ...
    setup() {
        ...
        store.dispatch("getCustomerDetail", route.params.id).then(() => {
            if (store.state.customersState.customerDetail) {
                ...
                phoneNumber.value = store.state.customersState.customerDetail.phoneNumber
                if (store.state.customersState.customerDetail.company) {
                    companyId.value = store.state.customersState.customerDetail.company.id
                    companyName.value = store.state.customersState.customerDetail.company.name
                }
            }
        })
        ...
        return {
            name, phoneNumber, search, companyId, companyName, customerId
        }
